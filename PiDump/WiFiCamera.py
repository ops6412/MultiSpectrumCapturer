...
MICRO_SERVO_PIN = 18
MICRO_START_ANGLE = 160  # Start at 140°
...
SPI_SPEED_HZ = 100000  # 1 MHz SPI speed
...
def read_adc_mcp3201():
    ...
    adc_value = (raw16 >> 3) & 0x1FFE  # Extract the 12-bit ADC result
    return adc_value
...
try:
    ...
    while micro_angle >= MICRO_END_ANGLE:
        ...
        print("Moving micro servo down by 5° after reverse sweep...")
        micro_angle -= 5
        if micro_angle < MICRO_END_ANGLE:
            break  # Stop movement when reaching 80°
        set_servo_angle(micro_servo, micro_angle)
    ...
    # Normalize data for OpenCV color mapping
    min_val, max_val = -80, 20  # dBm range
    ...
    # Apply OpenCV colormap (Jet colormap with flipped colors for yellow-to-blue effect)
    rf_colormap = cv2.applyColorMap(255 - rf_matrix_uint8, cv2.COLORMAP_JET)

    # Resize image to make it larger
    scale_factor = 2  # Increase image size by a factor of 2
    rf_colormap_resized = cv2.resize(rf_colormap, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_CUBIC)

    # Display RF matrix as an image centered in the OpenCV window
