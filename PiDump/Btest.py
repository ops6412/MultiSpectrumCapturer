...
def handle_third_button(button):
    ...
    if high_count >= 3:
        lower = set_value
        print(f"Third button was held. Lower set to: {lower}")
    elif 0 < high_count < 3:
        upper = set_value
        print(f"Third button was pressed. Upper set to: {upper}")
    else:
        print("Third button was not pressed or held.")

# Main loop to update all buttons every 500ms
try:
    while True:
        handle_button_state(button1, "Button 1", "add")
        handle_button_state(button2, "Button 2", "subtract")
        handle_third_button(button3)
